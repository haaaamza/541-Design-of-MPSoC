...
void bus ::Request(unsigned int mst_id, unsigned int addr, unsigned int op, unsigned int len){
    ...
    while (que.front().mst_id != mst_id && que.front().op != op && que.front().addr != addr && que.front().len !=len){ //ensure synchronization of request for R/W
        wait(6.66, SC_NS);
    }
    wait(6.66, SC_NS);//Two CK for Request.
    wait(6.66, SC_NS);
...
void bus :: ReadData(unsigned int& data){
    ...
    if (sc_fifo.size() <= que.front().len){
        ...
        sent_count ++;
        if (sent_count>que.front().len){ //checking if we pass the write amt of data.
            cout<<"Incorrect amount of data passed to Read!";
            sc_stop();
        }
        //_____________READING DATA___________
        data = sc_fifo.front();
        sc_fifo.pop();
    }
    wait(6.66, SC_NS);
    wait(6.66, SC_NS); //READ 2CK
...
void bus:: SendReadData(unsigned int data){
    wait(6.66, SC_NS); //For Synchronization
    if (sc_fifo.size() < que.front().len){
        wait(6.66, SC_NS);
        sc_fifo.push(data);
    }
...
void bus:: ReceiveWriteData(unsigned int &data){
    if (sc_fifo.size() <= que.front().len){
        while (sc_fifo.empty()){
            wait(6.66, SC_NS); //1CK //:Since we are alwayrs forced to wait atleast once.
        }
        sent_count++;
        if (sent_count>que.front().len){ //checking if we pass the write amt of data.
            cout<<"Incorrect amount of data passed from Write!";
            sc_stop();
        }
        //______PASSING DATA______
        data = sc_fifo.front();
        sc_fifo.pop();
    }
}
